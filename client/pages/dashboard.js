          ...
          <div className={style.UserContent}>
            ...
            <div className={style.List}>
              {!isempty(ItemList) && ItemType === "blog" ? ItemList.map(item => (
                ...
                <ItemBlockSm buttons={[{color: '#3f3f3f', text: 'Read', newWindow: true, link: `/blog/${item.post_url}.${item.blog_id}`}, {color: '#3A2F8F', text: 'Edit', newWindow: true, link: `/blog/edit/${item.post_url}.${item.blog_id}`}, {color: '#E7292A', text: 'Delete', newWindow: true, link: ''}]} key={item.blog_id} Published={item.published} Author={item.author} Image={item.image !== null ? `${apiUrl}/images/${item.image}.${item.image_ext}` : null} Title={item.title} Date={getStringDate(item.created_date)} ButtonNewWindow={false} />
              )) : null}
